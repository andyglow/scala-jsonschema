...
object AsPlay {
  ...
  object Adapter extends LowPriorityAdapter {
    ...
    implicit val arrAdapter: Aux[arr, JsArray] = make(x => JsArray { x.value.toSeq map { AsPlay(_) } })
    implicit val objAdapter: Aux[obj, JsObject] = make(x => JsObject { x.value.toMap mapV { AsPlay(_) } })
  }
}
