...
object Util {
  ...
  /** Basically a fast, efficient `.flatten` or `mkString` for nested iterators
    ...
    *
    * Also, ++ didn't seem to be sufficiently lazy, so it was forcing things earlier than it really needed to. It isn't documented anywhere how lazy it's meant to be, whereas `ConcatIterator` here is
  ...
  def escapeChar(c: Char, sb: StringBuilder, unicode: Boolean = true) = (c: @switch) match {
    case '"'  => sb.append("\\\"")
    case '\\' => sb.append("\\\\")
    case '\b' => sb.append("\\b")
    case '\f' => sb.append("\\f")
