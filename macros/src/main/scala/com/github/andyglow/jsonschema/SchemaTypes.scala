...
private[jsonschema] trait SchemaTypes { this: UContext with UCommons =>
  ...
  object SchemaType {
    ...
    case class Enum(tpe: Type, values: Seq[(Tree, Option[String])], extra: Extra = Extra()) extends SchemaType { type Self = Enum; def prefix = q"${N.Schema}.`enum`.of[$tpe](..${values collect { case (tree, _) => tree }})"; def withExtra(x: SchemaType.Extra) = copy(extra = x) }
