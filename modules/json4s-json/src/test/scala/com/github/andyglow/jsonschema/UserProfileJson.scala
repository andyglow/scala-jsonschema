...
object UserProfileJson {
  ...
  implicit val CredentialsW: Writer[Credentials] = new Writer[Credentials] {
    override def write(o: Credentials): JValue =
      JObject("login" -> JString(o.login), "password" -> JString(o.password))
  }

  implicit val NotesW: Writer[Notes] = new Writer[Notes] {
    override def write(o: Notes): JValue =
      JObject("head" -> JString(o.head), "tail" -> JArray(o.tail map JString.apply))
  }
