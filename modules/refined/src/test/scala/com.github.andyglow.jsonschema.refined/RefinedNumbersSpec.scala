...
class RefinedNumbersSpec extends AnyFunSuite {
  ...
  test("int > n") {
      schema[Int Refined Greater[W.`20`.T]] shouldBe `integer`.withValidation(`exclusiveMinimum` := 20)
  }
  ...
  test("double positive") {
      schema[Double Refined Positive] shouldBe `number`[Double].withValidation(`exclusiveMinimum` := 0)
  }

  test("double negative") {
      schema[Double Refined Negative] shouldBe `number`[Double].withValidation(`exclusiveMaximum` := 0)
  }
  ...
  test("double > n") {
      schema[Double Refined Greater[W.`20`.T]] shouldBe `number`[Double].withValidation(`exclusiveMinimum` := 20)
  }

  test("double < n") {
      schema[Double Refined Less[W.`20`.T]] shouldBe `number`[Double].withValidation(`exclusiveMaximum` := 20)
  }

  test("double <= n") {
      schema[Double Refined GreaterEqual[W.`20`.T]] shouldBe `number`[Double].withValidation(`minimum` := 20)
  }

  test("double >= n") {
      schema[Double Refined LessEqual[W.`20`.T]] shouldBe `number`[Double].withValidation(`maximum` := 20)
  }

  test("double mod n") {
      schema[Double Refined Divisible[W.`20`.T]] shouldBe `number`[Double].withValidation(`multipleOf` := 20)
  }
}
