...
class RefinedStringsSpec extends AnyFunSuite {
  ...
  test("trimmed") {
    schema[String Refined Trimmed] shouldBe `string`.withValidation(
      `pattern` := """^(?!\s)[\S ]*(?<!\s)$"""
    )
  }

  test("uuid") {
    schema[String Refined Uuid] shouldBe `string`.withValidation(
  ...
  test("startsWith") {
    schema[String Refined StartsWith[W.`"Foo"`.T]] shouldBe `string`.withValidation(
      `pattern` := "^Foo.*$"
    )
  }

  test("endsWith") {
    schema[String Refined EndsWith[W.`"Bar"`.T]] shouldBe `string`.withValidation(
      `pattern` := "^.*Bar$"
    )
  }

  test("matchesRegexp") {
    schema[String Refined MatchesRegex[W.`"[a-z]*"`.T]] shouldBe `string`.withValidation(
      `pattern` := "[a-z]*"
    )
  }
