...
trait LowPriorityCatsSupport extends ScalaVersionSpecificLowPriorityCatsSupport {
  ...
  implicit def nemStrVB[K, V]: Magnet[NonEmptyMap[K, V], Map[_, _]] =
    mk[NonEmptyMap[K, V], Map[_, _]]
  ...
  implicit def chainSchemaFromPredef[T](implicit p: Predef[T]): Predef[Chain[T]] =
  ...
  implicit def nelSchemaFromPredef[T](implicit p: Predef[T]): Predef[NonEmptyList[T]] =
    mkNEx[T, NonEmptyList](p.schema)

  implicit def nevSchemaFromPredef[T](implicit p: Predef[T]): Predef[NonEmptyVector[T]] =
    mkNEx[T, NonEmptyVector](p.schema)

  implicit def nesSchemaFromPredef[T](implicit p: Predef[T]): Predef[NonEmptySet[T]] =
    mkNEx[T, NonEmptySet](p.schema, unique = true)

  implicit def necSchemaFromPredef[T](implicit p: Predef[T]): Predef[NonEmptyChain[T]] =
