...
//    ratingSchema.refName
    ...
    json.Json.schema[User].asU(Draft04()) shouldEqual ujson.Obj(
      ...
      "properties" -> ujson.Obj(
        "rating" -> ujson.Obj(
          f"$$ref" -> ujson.Str("#/definitions/com.github.andyglow.jsonschema.AsUSpec.Rating")
        ),
      ...
      "definitions" -> ujson.Obj(
        "com.github.andyglow.jsonschema.AsUSpec.Rating" -> ujson.Obj(
          "type"                 -> ujson.Str("object"),
          "additionalProperties" -> ujson.False,
          "properties"           -> ujson.Obj("value" -> ujson.Obj("type" -> ujson.Str("integer"))),
...
//    ratingSchema.refName
    ...
    json.Json
      .schema[User]
      .asU(Draft06(id = "http://models.org/userProfile.json")) shouldEqual ujson.Obj(
      ...
      "properties" -> ujson.Obj(
        "rating" -> ujson.Obj(
          f"$$ref" -> ujson.Str("#com.github.andyglow.jsonschema.AsUSpec.Rating")
        ),
        "age"    -> ujson.Obj("type" -> ujson.Str("integer")),
        "active" -> ujson.Obj("type" -> ujson.Str("boolean"), "default" -> ujson.True),
        "name" -> ujson.Obj(
...
//    ratingSchema.refName
    ...
    json.Json
      .schema[User]
      .asU(Draft07(id = "http://models.org/userProfile.json")) shouldEqual ujson.Obj(
    ...
    json.Json.schema[UserProfile].asU(Draft04()) shouldEqual ujson.Obj(
      ...
      "properties" -> ujson.Obj(
        ...
        "role" -> ujson.Obj(
          "type"    -> "string",
          "default" -> "e-user",
          "enum"    -> ujson.Arr("e-admin", "e-manager", "e-user")
        ),
        "active" -> ujson.Obj(
          "type"    -> "string",
          "default" -> "On",
          "enum"    -> ujson.Arr("On", "Off", "Suspended")
        ),
        "enabledFeatures" -> ujson.Obj(
          "type"        -> "array",
          "uniqueItems" -> true,
          "default"     -> ujson.Arr("feature-0-name", "feature-1-name"),
          "items" -> ujson.Obj(
            "type" -> "string",
            "enum" -> ujson.Arr("feature-0-name", "feature-1-name", "feature-2-name")
          )
        ),
        "credentials" -> ujson.Obj(
          "type"                 -> "object",
          "additionalProperties" -> false,
...
object AsUSpec {
  ...
  case class User(name: Name, rating: Rating, age: Int, active: Boolean = true)
