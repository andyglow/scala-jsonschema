...
object AsU {
  ...
  implicit class UJsonSchemaOps[T](val x: Schema[T]) extends AnyVal {
    ...
    def asU[V <: Version](v: V)(implicit asValue: AsValueBuilder[V]): ujson.Obj = AsU(
      AsValue.schema(x, v)
    )
  }
  ...
  object Adapter extends LowPriorityAdapter {
    ...
    implicit val falseAdapter: Aux[`false`.type, ujson.False.type] =
      make(_ => ujson.False, _ => `false`)
    implicit val numAdapter: Aux[num, ujson.Num] =
      make(x => ujson.Num(x.value.toDouble), x => num(x.value))
    implicit val strAdapter: Aux[str, ujson.Str] = make(x => ujson.Str(x.value), x => str(x.value))
    implicit val arrAdapter: Aux[arr, ujson.Arr] =
      make(x => x.value map { adapt(_) }, x => arr { x.value map { unadapt(_) } })
    implicit val objAdapter: Aux[obj, ujson.Obj] =
      make(x => x.value.toMap mapV { adapt(_) }, x => obj { x.value.toMap mapV { unadapt(_) } })
  }
